...
const FILTERS = ["Wszystkie", "Porady", "Inspiracje", "Aktualności", "Technologie"] as const;
...
export default function BlogContent({ posts }: { posts: BlogPost[] }) {
  ...
  const filtered = active === "Wszystkie" ? posts : posts.filter((p) => p.category === active);
          ...
          {FILTERS.map((cat) => (
            <button
              ...
              className={`flex items-center gap-2 whitespace-nowrap px-3 py-2 text-sm transition-colors md:text-lg lg:text-xl ${
                active === cat ? "font-bold text-dark" : "text-dark/60 hover:text-dark"
              }`}
            >
              <span
                className={`flex size-4 shrink-0 items-center justify-center border transition-colors ${
                  active === cat ? "border-pink bg-pink" : "border-dark/30"
                }`}
              >
                {active === cat && (
          ...
          <Link href={`/blog/${featured.slug}`} className="group block">
            ...
            <div className="mt-6 md:mt-8">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className="shrink-0 text-sm text-dark/60 md:text-base">{featured.date}</span>
              </div>
              <p className="mt-4 text-sm leading-relaxed text-dark md:text-base lg:text-lg">{featured.excerpt}</p>
              <div className="mt-4 inline-flex flex-col items-start gap-1">
                <span className="text-sm text-dark md:text-base">Czytaj artykuł</span>
              ...
              {gridPosts.map((post) => (
                <Link key={post.slug} href={`/blog/${post.slug}`} className="group block">
                  ...
                  <div className="mt-4 flex items-center justify-between gap-2">
                    <span className="bg-gray-200/50 px-3 py-1.5 text-sm text-dark">{post.category}</span>
