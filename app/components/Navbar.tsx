...
function mediaUrl(media: { url: string; formats?: Record<string, { url: string }> } | null, format?: "small" | "medium"): string {
	if (!media) return "";
	const candidate = format && media.formats?.[format]?.url ? media.formats[format].url : media.url;
...
export default function Navbar({ global, navigation }: Props) {
	const pathname = usePathname();
	const menuItems = navigation.mainMenu;
	const productCategories = navigation.productCategories;
		...
		<nav className="border-b border-gray-100 bg-white transition-all duration-300">
			<div className={`mx-auto flex max-w-[1440px] items-center justify-between px-3 md:px-5 transition-all duration-300 ${scrolled ? "py-1 md:py-1.5" : "py-2.5 md:py-3"}`}>
					...
					{menuItems.map((item) =>
						item.hasDropdown ? (
							...
							>
								...
								<div className={`absolute left-0 top-full z-50 pt-2.5 transition-all duration-200 ${dropdownOpen ? "visible translate-y-0 opacity-100" : "invisible -translate-y-1 opacity-0"}`}>
									<div className="flex gap-0 border border-gray-100 bg-white shadow-lg shadow-dark/5">
										{productCategories.map((group) => (
											<div key={group.group} className="min-w-[240px] border-r border-gray-100 py-3 last:border-r-0">
												...
												{group.items.map((product, idx) => (
													...
													>
														{product.image && (
															<Image src={mediaUrl(product.image, "small")} alt={product.name} width={28} height={28} className="size-7 object-contain" />
														)}
														{product.name}
													</Link>
												))}
							...
							<Link
								key={item.label}
								href={item.href}
								className={`whitespace-nowrap transition-colors hover:text-pink ${hoverUnderline} ${item.bold ? "font-semibold" : "font-normal"} ${isActive(item.href) ? "text-pink after:!w-full" : ""}`}
							>
								{item.label}
							</Link>
						),
				...
				<button
					...
					aria-label="Menu"
				>
					<span className={`block h-0.5 w-6 transition-all duration-300 ${open ? "translate-y-2 rotate-45 bg-white" : "bg-dark"}`} />
					<span className={`block h-0.5 w-6 transition-all duration-300 ${open ? "scale-x-0 opacity-0 bg-white" : "bg-dark"}`} />
					<span className={`block h-0.5 w-6 transition-all duration-300 ${open ? "-translate-y-2 -rotate-45 bg-white" : "bg-dark"}`} />
				</button>
			</div>

			<div className={`fixed inset-0 z-50 flex flex-col bg-dark transition-all duration-500 ease-in-out lg:hidden ${open ? "visible opacity-100" : "invisible opacity-0"}`}>
				<div className="flex items-center px-3 py-2.5 md:px-5 md:py-3">
					<Link href="/" onClick={() => setOpen(false)} className="shrink-0">
						<Image src="/images/logo.svg" alt={global.companyName} width={125} height={82} className="h-[50px] w-auto brightness-0 invert" />
					</Link>
				</div>
						...
						item.hasDropdown ? (
							<div key={item.label}>
								<button
									...
									className={`flex w-full items-center justify-between border-b border-white/10 py-2.5 text-base text-white hover:pl-2 hover:text-pink sm:text-lg md:py-3 md:text-xl ${item.bold ? "font-semibold" : "font-light"}`}
									style={{
								...
								>
									{item.label}
									<svg width="14" height="14" viewBox="0 0 14 14" fill="none" className={`shrink-0 transition-transform duration-200 ${mobileProductsOpen ? "rotate-180" : ""}`}>
									...
									<div className="overflow-hidden">
										<div className="flex flex-col border-b border-white/10 py-2">
											{productCategories.map((group) => (
												<div key={group.group}>
													<button
														onClick={() => setMobileCategoryOpen(mobileCategoryOpen === group.group ? null : group.group)}
													...
													>
														{group.group}
														<svg width="10" height="10" viewBox="0 0 10 10" fill="none" className={`shrink-0 transition-transform duration-200 ${mobileCategoryOpen === group.group ? "rotate-180" : ""}`}>
															<path d="M2 4L5 7L8 4" stroke="currentColor" strokeWidth="1.5" />
														</svg>
													</button>
													<div className={`grid transition-all duration-300 ${mobileCategoryOpen === group.group ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
														<div className="overflow-hidden">
															{group.items.map((product, idx) => (
																<Link
																	key={product.name}
							...
							<Link
								...
								href={item.href}
								onClick={() => setOpen(false)}
								className={`border-b border-white/10 py-2.5 text-base text-white hover:pl-2 hover:text-pink sm:text-lg md:py-3 md:text-xl ${item.bold ? "font-semibold" : "font-light"}`}
								style={{
									transitionProperty: "opacity, transform, color, padding-left",
									transitionDuration: "300ms",
									transitionTimingFunction: "ease-out",
